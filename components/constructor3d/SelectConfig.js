import {
	Mesh, Group, Math, MeshStandardMaterial, PlaneGeometry, BoxGeometry,
} from "three"

const selectBox = ({
	width, height, depth, x, y, z, rotate, pos,
}) => {
	...
	const line = new PlaneGeometry(1, 0.2)
	const group = new Group()
	group.add(sideLeft)

	const mesh1 = new Mesh(line, meshMaterial)
	const mesh2 = new Mesh(line, meshMaterial)
	const mesh3 = new Mesh(line, meshMaterial)
	...
	const mesh4 = new Mesh(line, meshMaterial)
	const mesh5 = new Mesh(line, meshMaterial)
	const mesh6 = new Mesh(line, meshMaterial)
	...
	const mesh7 = new Mesh(line, meshMaterial)
	const mesh8 = new Mesh(line, meshMaterial)
	const mesh9 = new Mesh(line, meshMaterial)

	mesh7.position.x = 0.5
	mesh8.rotateY(Math.degToRad(-90))
	mesh8.position.z = -0.5
	mesh9.rotateZ(Math.degToRad(-90))
	mesh9.position.y = 0.5
	...
	const mesh10 = new Mesh(line, meshMaterial)
	const mesh11 = new Mesh(line, meshMaterial)
	const mesh12 = new Mesh(line, meshMaterial)

	mesh10.position.x = -0.5
	mesh11.rotateY(Math.degToRad(-90))
	mesh11.position.z = -0.5
	mesh12.rotateZ(Math.degToRad(-90))
	mesh12.position.y = 0.5
