import {
	BoxGeometry, BufferGeometry, BufferAttribute, Group, CylinderGeometry, Math, Mesh, MeshStandardMaterial, Object3D,
} from "three"
...
const boxAngularTop = () => {
	const bodyWidth = 4.2 * 2
	const bodyHeight = 8
	const bodyDepth = bodyWidth

	const bD = 4.2
	const sP = 1 // катет дальнего угла
	...
	const sideV = new Mesh(geometry, material)

	const sideLeft = new Mesh(gSideLR, material)
	const sideRight = new Mesh(gSideLR, material)
	const sideBackR = new Mesh(gSideBack, material)
	const sideBackL = new Mesh(gSideBack, material)
	const sideBottom = new Mesh(geometry, material)
	const sideBTop = new Mesh(geometry, material)
...
const top_400_800 = () => {
	...
	const gSideLR = new BoxGeometry(boxDepth, boxHeight, sideDepth)
	const gSideBack = new BoxGeometry(boxWidth - sideDepth * 2, boxHeight, sideDepth)
	const gSideBottom = new BoxGeometry(boxWidth - sideDepth * 2, boxDepth, sideDepth)

	const sideLeft = new Mesh(gSideLR, material)
	const sideRight = new Mesh(gSideLR, material)
	const sideBack = new Mesh(gSideBack, material)
	const sideBottom = new Mesh(gSideBottom, material)
	const sideBTop = new Mesh(gSideBottom, material)
	const sideShelf = new Mesh(gSideBottom, material)
	const facadeLeft = new Group()
	...
	const objFacadeRight = new Group()
	objFacadeRight.add(facadeRight)
	facadeRight.name = "doorBox"
	facadeRight.position.x = -boxWidth / 4 + sideDepth / 2
	objFacadeRight.position.x = boxWidth / 2 - sideDepth / 2
...
const bottom_400_800 = () => {
	...
	const boxWidth = bodyWidth
	const boxHeight = bodyHeight - legsHeight
	const boxDepth = bodyDepth

	const gSideLR = new BoxGeometry(boxDepth, boxHeight, sideDepth)
	const gSideBack = new BoxGeometry(boxWidth - sideDepth * 2, boxHeight, sideDepth)
	const gSideBottom = new BoxGeometry(boxWidth - sideDepth * 2, boxDepth, sideDepth)
	const gSideTop = new BoxGeometry(boxWidth - sideDepth * 2, sideTop, sideDepth)
	const gLegFront = new BoxGeometry(boxWidth, legsHeight, sideDepth)
	...
	const sideBottom = new Mesh(gSideBottom, material)
	const sideShelf = new Mesh(gSideBottom, material)
	const sideTopFront = new Mesh(gSideTop, material)
	const sideTopBack = new Mesh(gSideTop, material)
	const facadeLeft = new Group()
	const facadeRight = new Group()
	const legFront = new Mesh(gLegFront, material)
	...
	const drawerHeight = 2.2 // высота выдвижного ящика
	...
	const drawerBottom = new Mesh(drawerBottomGeometry, material)
	const drawerSideR = new Mesh(drawerSideGeometry, material)
	const drawerSideL = new Mesh(drawerSideGeometry, material)
	const drawerSideBack = new Mesh(drawerSideBackGeometry, material)
...
const boxAngularFloor = () => {
	...
	const gSideTop = new BoxGeometry(boxWidth - sideDepth * 2, sideTop, sideDepth)
	const gLegFront = new BoxGeometry(boxWidth, legsHeight, sideDepth)
	const gLegFrontMini = new BoxGeometry(legFrontMargin + sideDepth, legsHeight, sideDepth)
	...
	const facadeLeft = new Group()
	const legFront = new Mesh(gLegFront, material)
	const legFrontMini = new Mesh(gLegFrontMini, material)

	const legLeft = new Mesh(gLegs, legMaterial)
	const legRight = new Mesh(gLegs, legMaterial)

	const objFacadeLeftRight = new Object3D()
