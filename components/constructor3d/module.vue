...
const LEGS_HEIGHT = 1
const GAP_FACADE = 0.1
const SIDE_DEPTH = 0.3
const TABLE_TOP_DEPTH = 7
...
export default {
	...
	computed: {
		...
		bottomPaddingRight() {
			...
			return paddingCases + (angularPadding || paddingLeft)
		},
		bottomPaddingLeft() {
		...
		cases: {
			...
			set(v) {
				...
				if (kitchen && kitchen.order && kitchen.order.cases) {
					const mapped = v.map(({
						uuid, userData: {
							form, material, size, color, value, price, sort,
						},
					}) => ({
						uuid,
						form,
	...
	methods: {
		...
		calcCasePosition(place, caseSort) {
			switch (place) {
				case "bottomRight": {
					...
					return z + (angularPadding || paddingLeft)
				}
				case "bottomLeft": {
		...
		setCasesPosition() {
			this.scene.children.forEach((el) => {
				const {
					position: { x, y, z }, place, userData: { sort, width }, name,
				} = el
			...
			if (selectedObject && this.selectedCase) {
				...
				if (leftIdx > -1) {
					this.bottomLeft
						...
						.forEach((el) => {
							...
							if (!isReplace) el.position.set(el.position.x + width + (padding || 0), y, z)
				...
				if (rightIdx > -1) {
					this.bottomRight
						...
						.forEach((el) => {
							const { x, y, z } = el.position
							if (!isReplace) el.position.set(x, y, z - width - (padding || 0))
							if (!isReplace) el.userData.sort -= 1
						})
		...
		addBottomRightToScene(isReplace) {
			const body = this.bodyCase.clone()
		...
		addTableTop() {
			...
			const {
				height, type, color, maxWidth, url,
			} = this.tableTopConfig
			...
			} else if (fractionLeft) addNewTableTopLeft(0, fractionLeft, height, type, url, 0)
		},
		initWalls() {
		...
		selectCase() {
			...
			function onPointerDown(event) {
				...
				if (intersects.length > 0) {
					...
					const controlActionName = findActionName(object)
